...
function AdminComplaint() {
  ...
  const axiosGet = async () => {
    const response = await axios.get(
      `https://giftshop-1636920008628.azurewebsites.net/admin/showAllComplaints`
    );
    // console.log("Admin")
  ...
  const axiosUpdateComplaint = async (key) => {
    try {
      const response = await axios.put(
        `https://giftshop-1636920008628.azurewebsites.net/admin/updateComplaint/${key}`
      );
      console.log(response);
  ...
  const handleEdit = (
    ...
    alert("Edit?");
    console.log(
      `key: ${key}, Username: ${userName}, name: ${nameOfUser}, email: ${userEmail}, passwd: ${userPassword}`
    );
    setUser(addUserData);
  ...
  const handleDelete = (e, key) => {
    e.preventDefault();
    console.log(key);
    alert("Do you want to change the status to Resolved?");
    // setUser(users[key])
    axiosUpdateComplaint(users[key].compId);
  };
                    ...
                    <tr>
                      ...
                      <td>
                        {/* <input placeholder={nameofUser} type="text" id="form12" class="form-control"
                                            ...
                                            /> */}
                        <input
                          placeholder={compStatus ? "Resolved" : "Pending"}
                          type="text"
                          id="form12"
                          class="form-control"
                          // onChange={e => setNameOfUser(e.target.value)}
