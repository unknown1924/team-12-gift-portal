...
public class AdminController {
	...
	@PutMapping("/admin/updateUserByName/{username}")
	...
	@PutMapping("/admin/updateOrderProcessing/{id}")
	...
	@PutMapping("/admin/updateOrderDispatched/{id}")
	...
	//Update/Resolve a complaint by ID
		...
		public boolean updateComplaint(@PathVariable("id") long id) {
			try {
				...
				Complaints compFound = compRepo.findById(id).get();
				comp.setCompId(compFound.getCompId());
				comp.setCompMessage(compFound.getCompMessage());
				comp.setCompStatus(true);
				comp.setOrderId(compFound.getOrderId());
				compRepo.save(comp);
				List<Users> lst = userRepo.findAll();
				lst.forEach((user) -> {
	...
	@DeleteMapping("/admin/deleteUserByUserName/{username}")
